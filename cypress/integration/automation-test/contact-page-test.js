...
it('Testing layouts in Contacts', () => {
    cy.log('contact-us button click check')
...
it('contact us test with fillings', () => {
    ...
    cy.get('[id=contact-link]').click()
    cy.get('[id=id_contact]').should('have.value', '0')
...
it('File upload test only', () => {

    cy.log('contact us check')
    cy.get('[id=contact-link]').click()
    const imageFile = 'test.png';
    cy.get('[id=fileUpload]').attachFile(imageFile);
...
it.only('Full fill form with including file upload', () => {
